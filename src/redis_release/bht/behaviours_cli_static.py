    ...
    def update(self) -> Status:
        ...
        if self.package_meta.release_type is not None:
            ...
            self.feedback_message = (
                f"Detected release type for cli-static: {self.package_meta.release_type}"
            )
        else:
...
class NeedToReleaseCliStatic(LoggingAction):
    """Check if redis-cli-static package needs to be released."""
    ...
    def update(self) -> Status:
        ...
        if self.release_version is not None:
            if self.release_version.major < 8:
                self.feedback_message = (
                    f"Skip release for cli-static {str(self.release_version)} < 8.0"
                )
                result = Status.FAILURE
            else:
                self.feedback_message = (
                    f"Need to release cli-static version {str(self.release_version)}"
                )
                result = Status.SUCCESS
        else:
