    ...
    def update(self) -> Status:
        ...
        if self.package_meta.release_type is not None:
            ...
            self.feedback_message = (
                f"Release type for RPM (from state): {self.package_meta.release_type}"
            )
        elif self.release_version is not None:
            # RPM only publishes GA versions
            ...
            result = Status.SUCCESS
            self.feedback_message = (
                f"Detected release type for RPM: {self.package_meta.release_type}"
            )
        else:
...
class NeedToReleaseRPM(LoggingAction):
    ...
    def update(self) -> Status:
        ...
        if self.release_meta.tag is None:
            ...
            result = Status.FAILURE
        if self.release_meta.tag == "unstable":
            self.feedback_message = "Skip unstable release for RPM"
            result = Status.FAILURE
